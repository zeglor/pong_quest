...
class TwoPlatforms:
	...
	def create(cl, platform_type, *parms, **kparms):
		...
		elif platform_type == 'gravity_single_top':
			return TwoPlatformsGravitySingleTop(platform_type, *parms, **kparms)
		elif platform_type == 'gravity_single_bot':
			return TwoPlatformsGravitySingleBot(platform_type, *parms, **kparms)
	...
	def add(self, group, *groups):
		if self.pl_top is not None:
			self.pl_top.add(group, *groups)
		if self.pl_bot is not None:
			self.pl_bot.add(group, *groups)

class TwoPlatformsGravity(TwoPlatforms):
	def __init__(self, platform_type, dynamic_objects):
		#init top platform
		self.pl_top = Platform.create('gravity', pos = (20, 60), keys = (pg.K_a, pg.K_d), 
			dynamic_objects = dynamic_objects, top=True)
		#init bottom platform
		self.pl_bot = Platform.create('gravity',pos = (20, DISPLAY_SIZE[1]-60), keys = (pg.K_LEFT, pg.K_RIGHT), 
			dynamic_objects = dynamic_objects)

class TwoPlatformsGravitySingleTop(TwoPlatforms):
	def __init__(self, platform_type, dynamic_objects):
		#init top platform
		self.pl_top = Platform.create('gravity', pos = (20, 60), keys = (pg.K_a, pg.K_d), 
			dynamic_objects = dynamic_objects, top=True)
...
class TwoPlatformsGravitySingleBot(TwoPlatforms):
	def __init__(self, platform_type, dynamic_objects):
		self.pl_top = None
		...
		if activate == True:
			for o in self.dynamic_objects:
				if o.rect.center[0] >= self.rect.left and o.rect.left <= self.rect.center[0]:
					...
					if o in self.affected_objects.keys():
						o.speed_y -= self.delta_speed#self.affected_objects[o][1]
			...
			if len(self.affected_objects) > 0:
				for o, speed in self.affected_objects.values():
					o.speed_y -= self.delta_speed #speed
